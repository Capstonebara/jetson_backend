...
# Changelog:
# 13/11/2025: Add highlighting for messages
...
async def main() -> None:
    ...
    parser.add_argument(
        ...
        action="append",
        default=[],
        help="Regular expression matching the names of the source files to exclude.",
    )
    if yaml:
        parser.add_argument(
            "-export-fixes",
    ...
    parser.add_argument(
        "-query-driver", 
        default="c++", 
        help="Compiler to query for system includes (default: c++). Set to 'none' to disable."
    )

    args = parser.parse_args()
    if args.query_driver and args.query_driver.lower() != "none":
        if not args.quiet:
            print(f"Auto-detecting system includes from '{args.query_driver}'...", file=sys.stderr)
        
        detected_includes = get_system_includes(args.query_driver)
        
        if detected_includes:
            if not args.quiet:
                print(f"Found {len(detected_includes)} system include paths.", file=sys.stderr)
            # Thêm các paths tìm được vào extra_arg để clang-tidy sử dụng
            args.extra_arg.extend(detected_includes)
        else:
            if not args.quiet:
                print(f"Warning: Could not detect system includes from '{args.query_driver}'.", file=sys.stderr)
    ...
    if args.exclude:
        try:
            # Combine multiple exclude patterns into one OR regex, carefully wrapping each in ()
            # to prevent operator precedence issues.
            combined_pattern = "|".join(f"({p})" for p in args.exclude)
            exclude_re = re.compile(combined_pattern)
            
            # Giữ lại các file KHÔNG khớp với regex exclude
            files = {f for f in files if not exclude_re.search(f)}
        except re.error as e:
            print(f"Error: Invalid regex for -exclude: {e}", file=sys.stderr)
            sys.exit(1)
    ...
    try:
        ...
        if not args.hide_progress:
            passed_results.sort(key=lambda r: r.filename)
            for i, result in enumerate(passed_results):
                progress = f"[{i + 1: >{len(f'{len(passed_results)}')}}/{len(passed_results)}]"
                runtime = f"[{result.elapsed:.1f}s]"
                status = f"{Colors.GREEN}[PASSED]{Colors.RESET}"

                print(f"{status} {progress}{runtime} {' '.join(result.invocation)}")

        if failed_results:
            print(f"\n{Colors.RED}--- CLANG-TIDY SUMMARY: {len(failed_results)} FAILED ---{Colors.RESET}", file=sys.stderr)

            failed_results.sort(key=lambda r: r.filename)
            for i, result in enumerate(failed_results):
                progress = f"[{i + 1: >{len(f'{len(failed_results)}')}}/{len(failed_results)}]"
                runtime = f"[{result.elapsed:.1f}s]"
                status = f"{Colors.RED}[FAILED]{Colors.RESET}"

                if not args.hide_progress:
                    print(f"{status} {progress}{runtime} {' '.join(result.invocation)}", file=sys.stderr)

                if result.stdout:
                    print(f"{Colors.YELLOW}{result.stdout}{Colors.RESET}", end=("" if result.stderr else "\n"), file=sys.stderr)
                if result.stderr:
...
def get_system_includes(compiler_cmd: str) -> List[str]:
    """
    Simulates --query-driver behavior:
    Runs the compiler with -E -v to extract default system include paths.
    """
    try:
        # 1. Kiểm tra compiler có tồn tại không
        if not shutil.which(compiler_cmd):
            print(f"Warning: Compiler '{compiler_cmd}' not found in PATH.", file=sys.stderr)
            return []

        # 2. Ép môi trường về tiếng Anh để dễ parse output (LC_ALL=C)
        env = os.environ.copy()
        env["LC_ALL"] = "C"

        # 3. Chạy lệnh magic
        cmd = [compiler_cmd, "-E", "-x", "c++", "-", "-v"]
        ...
        result = subprocess.run(
            ...
            text=True, 
            check=False,
            env=env # <--- QUAN TRỌNG: Dùng env đã ép tiếng Anh
        )
        ...
        for line in output.splitlines():
            # Dòng này phải khớp chính xác với output tiếng Anh của GCC/Clang
            ...
            if is_include_section:
                ...
                if os.path.isdir(path):
                    if "lib/gcc" in path and path.endswith("include"):
                         continue
                    includes.append(f"-isystem{path}")
        
        # 4. Debugging: Nếu không tìm thấy gì, hét lên để user biết
        if not includes:
            print(f"Warning: Run '{compiler_cmd} -E -v -x c++ /dev/null' manually to check output.", file=sys.stderr)
            print(f"Debug Output head: {output[:200]}...", file=sys.stderr)

        return includes

    except Exception as e:
        print(f"Error detecting system includes: {e}", file=sys.stderr)
        return []
