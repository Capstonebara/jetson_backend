...
int main(int argc, const char *argv[])
{
    ...
    std::string detector_path = args.get(
        "--detector_path", "/home/jetson/FaceRecognitionSystem/jetson/backend/model/ssdlite/ssdlite_320_cuda.ts");
    std::string extractor_path =
        args.get("--extractor_path",
                 "/home/jetson/FaceRecognitionSystem/jetson/backend/model/kanface/kanface_06_25_128_custom_cuda.ts");
    ...
    while (true)
    {
        ...
        if (frame_counter % skip_frame == 0)
        {
            ...
            const torch::Tensor &selected_boxes =
                detector.inference(preprocessed_input, detector_threshold).to(torch::kCPU);
            ...
            else
            {
                ...
                progress_time +=
                    std::chrono::duration_cast<std::chrono::milliseconds>(progress_time_end - progress_time_start)
                        .count();
                ...
                if (progress_time < progress_duration)
                {
                    ...
                    cv::rectangle(frame,
                                  cv::Point(x1, y1 - int(height * 0.028)),
                                  cv::Point(x2, y1 - 5),
                                  cv::Scalar(202, 162, 177));
                    int progress_bar_width =
                        static_cast<int>((progress_time / static_cast<float>(progress_duration)) * box.width);

                    // draw progress_bar
                    cv::rectangle(frame,
                                  cv::Point(x1, y1 - int(height * 0.028)),
                                  cv::Point(x1 + progress_bar_width, y1 - 5),
                                  cv::Scalar(0, 255, 255),
                                  cv::FILLED);
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        mean_embedding =
                            F::normalize(mean_embedding,
                                         F::NormalizeFuncOptions().p(2).dim(-1)); // always normalize because of ArcFace
                        ...
                        std::cout << "Found user: \"" << db.findName(index) << "\" at index: " << index
                                  << " Similarity: " << similarity << std::endl;

                        std::ostringstream ss;
                        ss << db.findName(index) << " " << std::fixed << std::setprecision(2) << similarity;
                        ...
                        if (similarity > extractor_threshold)
                        {
                            ...
                            cv::putText(
                                frame, ss.str(), cv::Point(x1, y1 - 5), cv::FONT_HERSHEY_SIMPLEX, 1, bbox_color, 1);
                        ...
                        else
                        {
                            ...
                            cv::rectangle(frame, box, bbox_color, 2);
                            cv::putText(
                                frame, "Unknown", cv::Point(x1, y1 - 5), cv::FONT_HERSHEY_SIMPLEX, 1, bbox_color, 1);
        ...
        else
        {
            if (!last_boxes.empty())
            {
                for (size_t i = 0; i < last_boxes.size(); ++i)
                {
                    ...
                    cv::putText(
                        frame, "", {last_boxes[i].x, last_boxes[i].y - 5}, cv::FONT_HERSHEY_SIMPLEX, 1, bbox_color, 2);
                }
            }
        }
