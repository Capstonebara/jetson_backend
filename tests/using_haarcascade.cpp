...
int main(int argc, char *argv[])
{
    ...
    for (int i = 1; i < argc; ++i)
    {
        ...
        else
        {
            std::cerr << "Usage: " << argv[0]
                      << " [--camera <id>] [--width <value>] [--height <value>] [--use-usb] [--no-cuda] "
                         "[--process-all] [--skip <n>] [--process-width <w>] [--process-height <h>]"
                      << std::endl;
            return -1;
        }
    }
    ...
    if (use_cuda)
    {
        try
        {
            cuda_face_cascade = cv::cuda::CascadeClassifier::create(
                "/home/jetson/opencv-4.5.2/data/haarcascades_cuda/haarcascade_profileface.xml");
    ...
    while (true)
    {
        ...
        if (process_this_frame)
        {
            ...
            cv::putText(
                frame, detect_text.str(), cv::Point(10, 60), cv::FONT_HERSHEY_SIMPLEX, 0.7, cv::Scalar(0, 255, 255), 2);
        }
        ...
        cv::putText(
            frame, faces_text.str(), cv::Point(10, 90), cv::FONT_HERSHEY_SIMPLEX, 0.7, cv::Scalar(0, 255, 255), 2);
