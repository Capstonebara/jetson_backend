...
Logger::Logger(const std::string &output_dir, VectorDB &db) : m_output_dir(output_dir), m_db(db) {}
...
std::string Logger::getCurrentTimestamp()
{
    ...
    oss << std::setfill('0') << now->tm_hour << ":" << now->tm_mday << ":" << (now->tm_mon + 1) << "."
        << (now->tm_year + 1900);
    return oss.str();
}
...
void Logger::logFaceAndTime(faiss::idx_t index, const cv::Mat &face_roi)
{
    ...
    {
        data_file << user_id << " " << m_device_id << " " << timestamp << " " << (m_isCheckIn ? "entry" : "exit")
                  << std::endl;
        data_file.close();
    }
    else
