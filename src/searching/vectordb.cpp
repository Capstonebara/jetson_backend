#include "searching/vectordb.h"
#include "searching/embedding_loader.h"
#include <cmath>
#include <iomanip>
#include <iostream>

VectorDB::VectorDB(const std::string &db_path, const int embedding_size)
    : m_users(embedding_loader::load_all_embeddings(db_path)), m_embedding_size(embedding_size),
      m_index(m_embedding_size)
{
    ...
    for (size_t user_idx = 0; user_idx < m_users.size(); ++user_idx)
    {
        ...
        std::cout << "adding " << num_vectors << " embeddings for user: " << user.name << " (id: " << user.id << ")"
                  << std::endl;
