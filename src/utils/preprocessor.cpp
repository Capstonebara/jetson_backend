...
Preprocessor::Preprocessor(float min_sharpness, int min_brightness, int max_brightness, float min_area_ratio,
                           float max_area_ratio)
    : m_min_sharpness_threshold(min_sharpness), m_min_brightness_threshold(min_brightness),
      m_max_brightness_threshold(max_brightness), m_min_face_area_ratio(min_area_ratio),
...
// target_size_ initialization removed
{
    ...
    {
        std::cerr << "Warning: min_brightness (" << min_brightness << ") is greater than or equal to max_brightness ("
                  << max_brightness << ")." << std::endl;
    }
    if ((1.0f / min_area_ratio) < 0.0f)
    {
        std::cerr << "Warning: 1 / min_area_ratio (" << min_area_ratio << ") is less than 0. Check min_area_ratio."
                  << std::endl;
    }
    if ((1.0f / max_area_ratio) > 1.0f)
    {
        std::cerr << "Warning: 1 / max_area_ratio (" << max_area_ratio << ") is greater than 1.0. Check max_area_ratio."
                  << std::endl;
    }
    if (1.0f / min_area_ratio >= 1.0f / max_area_ratio)
    {
        std::cerr << "Warning: min_area_ratio (" << min_area_ratio << ") is greater than or equal to max_area_ratio ("
                  << max_area_ratio << ")." << std::endl;
    }
}
...
std::tuple<cv::Mat, cv::Rect, bool> Preprocessor::preprocessBestFace(
    const cv::Mat &frame, const torch::TensorAccessor<float, 2> &boxes_accessor, int transforms_width,
